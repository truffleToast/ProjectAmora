								...
								<!-- 글 출력되는 카테고리 란 -->
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
										...
										//출력되는 곳
												...
												ArrayList<BoardDTO> list = boardDAO.getListRequest(pageNumber);							
...
<td><a id="sangsebogi" href="ViewCountUpService?com_Num=<%= list.get(i).getCom_Num() %>&com_likes=<%= list.get(i).getCom_Likes() %>"><%= list.get(i).getCom_Title() %> </a></td>
									...
									//페이지 이동하는 버튼: 이전 다음.
										...
										if (pageNumber != 1) {
									%>
										<a href="request.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arrow-left">이전</a>
									<%
										} if (boardDAO.nextPageRequest(pageNumber + 1)) {
									%>
										<a style ="border-color: #7297F7; background-color: #7297F7;" href="request.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arrow-left">다음</a>
									<%
										}
									%> 
									<%
													int n = (int)(boardDAO.getCountRequest() / 10 + 1);
													for (int i = 1; i <= n; i++) {
									%>
													<a href="request.jsp?pageNumber=<%=i%>">|<%=i%>|
													</a>
									<%
														}
													String col ="";

									...
									<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				                        ...
				                        BoardDAO dao = new BoardDAO();
											ArrayList<BoardDTO> list_Search = new ArrayList<>();
													if("title_content".equals(col)){list_Search = dao.getListRequest_Search_Title_Content(sentence, pageNumber);}
													else if("content".equals(col)){list_Search =dao.getListRequest_Search_Content(sentence, pageNumber);}
													else if("title".equals(col)){list_Search =dao.getListRequest_Search_Title(sentence, pageNumber);}
													else if("writer".equals(col)){list_Search = dao.getListRequest_Search_Writer(sentence, pageNumber);}
											for (int i = 0; i < list_Search.size(); i++) { %>
											<tr style ="background-color:white;">
												<td><%= list_Search.get(i).getCom_Category() %></td>
												<td><%= list_Search.get(i).getCom_Num() %></td>
<td><a id="sangsebogi" href="ViewCountUpService?com_Num=<%= list_Search.get(i).getCom_Num() %>&com_likes=<%= list_Search.get(i).getCom_Likes() %>"><%= list_Search.get(i).getCom_Title() %> </a></td>
							...
							<!-- 검색 버튼임. -->
								<div style="position:relative; left:40px; font-size:20px; margin-top: 20px; text-align: center; height: 25px;">
									<form action="requestSearch.jsp" method="get">
										<select name="col">
